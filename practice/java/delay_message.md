...
* 固定梯度延时：RocketMQ 4.x 与 5.x 版本均支持固定梯度的延迟消息，消息发送到之后，不会立即被消费，等待特定时间投递给真正的topic，默认配置“1s 5s 10s 30s 1m 2m 3m 4m 5m 6m 7m 8m 9m 10m 20m 30m 1h 2h” 18个级别，每个级别对应不同的延时时间，比如1对应1s，5对应1m。
* 指定延时时间：仅 5.x 版本支持，延时时间可以精确到毫秒，不限于固定档位。
            ...
            try {
                {
                    ...
                    // msg.setDelayTimeMs(1000); // 延时 1000 毫秒
                    // msg.setDeliverTimeMs(System.currentTimeMillis() + 30 * 60 * 1000) ; // 延时 30 分钟
                    SendResult sendResult = producer.send(msg);
                    System.out.printf("%s%n", sendResult);
                }
